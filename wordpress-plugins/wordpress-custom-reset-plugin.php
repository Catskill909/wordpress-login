...
/**
 * Register custom REST API endpoints for password reset and registration verification
 */
add_action('rest_api_init', function () {
    ...
    // Request registration verification code endpoint
    register_rest_route('wp-flutter/v1', '/request-registration-code', array(
        'methods' => 'POST',
        'callback' => 'wp_flutter_request_registration_code',
        'permission_callback' => function () {
            return true; // Public endpoint
        }
    ));
    ...
    register_rest_route('wp-flutter/v1', '/verify-registration', array(
        'methods' => 'POST',
        'callback' => 'wp_flutter_verify_registration',
        'permission_callback' => function () {
            return true; // Public endpoint
        }
    ));
 ...
 * @return WP_REST_Response
 */
function wp_flutter_request_registration_code($request)
{
    $email = sanitize_email($request->get_param('email'));
    ...
    // Store verification code in user meta with expiration time (30 minutes)
    $expiration = time() + (30 * 60); // 30 minutes from now
    update_user_meta($user->ID, 'registration_code', $verification_code);
    update_user_meta($user->ID, 'registration_code_expiration', $expiration);

    // Send verification code email
    $subject = 'Email Verification Code';
    $message = 'Thank you for registering. Use the following verification code to complete your registration: ' . $verification_code . "\n\n";
    $message .= 'This code will expire in 30 minutes.';
 ...
 * @return WP_REST_Response
 */
function wp_flutter_verify_registration($request)
{
    $email = sanitize_email($request->get_param('email'));
    $code = sanitize_text_field($request->get_param('code'));
    ...
    // Get stored verification code and expiration
    $stored_code = get_user_meta($user->ID, 'registration_code', true);
    $expiration = get_user_meta($user->ID, 'registration_code_expiration', true);
    ...
    delete_user_meta($user->ID, 'registration_code');
    delete_user_meta($user->ID, 'registration_code_expiration');
